...
### `extract_ha_cds`
Per-subtype rule (wildcard `{tree}`) that extracts full-length HA CDS sequences and metadata. Uses `scripts/extract_ha_cds.py`. Identifies HA accessions as those with segment 4 in the genome metadata OR gene-name in {"HA", "ha", "HA1"} in the annotation genes TSV (some older NCBI records lack the segment field but have the gene annotation). Joins with subtypes parsed from genomic FASTA headers, filters by the `subtype` regex in `config["trees"][tree]`, then extracts matching full-length hemagglutinin CDS entries (not HA1/HA2/sig_peptide fragments) from `cds.fna` in the zip. Sequences with ambiguous nucleotides (not ACTG) are dropped, sequences that fail Biopython `translate(cds=True)` are dropped, and sequences outside the CDS length range are dropped. The length range is read from a YAML file specified by `config["trees"][tree]["cds_length_range"]`, which has a `cds_length_range` key with a two-element list `[min, max]`; either bound can be `null` to impose no limit on that side. Outputs are sorted by date then strain. Output files are suffixed `_all` to indicate they are before any subsampling.
